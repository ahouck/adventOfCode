 ...
 * Problem
 * 
 * Part One: The newly-improved calibration document consists of lines of text; 
...
//Faster to hardcode the data than read a file
const data =
...
/**
 * Brute force of making two explicit sliders one going from the left and one from the right to find the 
...
data.forEach(line => {
    ...
    for (pointerA; pointerA < line.length; pointerA++) {
        const pointerChar = line.charAt(pointerA);
        if (!isNaN(pointerChar)) {
            first = pointerChar;
            break;
        } else {
            const substr = line.substring(0, pointerA + 1);
            digits.forEach((d, i) => {
    ...
    let pointerB = line.length -1;
    for (pointerB; pointerB > -1; pointerB--) {
        const pointerChar = line.charAt(pointerB);
        console.log(line, pointerB, pointerChar, line.substring(pointerB - 1, line.length)); 
        if (!isNaN(pointerChar)) {
            last = pointerChar;
            break;
        } else {
            const substr = line.substring(pointerB - 1, line.length);
            digits.forEach((d, i) => {
                if (substr.includes(d)) {
                    last = `${i}`;
                }
